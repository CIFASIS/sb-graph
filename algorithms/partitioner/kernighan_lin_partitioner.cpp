...
{
    auto f = [](auto& a, auto& b, const PWMap& map_1, const PWMap& map_2) {
    ...
    unsigned LMax)
{
    ...
    unsigned p_size_a = get_partition_size(partition_a, graph.get_node_weights());
    unsigned p_size_b = get_partition_size(partition_b, graph.get_node_weights());
    ...
    unsigned LMax)
{
    ...
    for (auto g : cost_matrix) {
        ...
        if (change) {
            ...
            size_t c_ab = get_c_ab(set_a_i, set_b_j, graph.map1(), graph.map2(), graph.get_edge_costs());
...
KLBipartResult kl_sbg_bipart_imbalance(const WeightedSBGraph& graph, Partition& partition_a,
    Partition& partition_b, unsigned LMin, unsigned LMax)
{
    ...
    vector<kl_sbg_partitioner_result>& gains)
{
    ...
    for (size_t i = 0; i < partitions.size(); i++) {
        ...
        if (adjacents.find(i) == adjacents.end()) {
            adjacents.insert_or_assign(i, look_for_adjacents(from_vector(partitions.at(i)), graph.map1(), graph.map2()));
        }

        for (size_t j = i + 1; j < partitions.size(); j++) {

            if (adjacents.at(i).intersection(from_vector(partitions.at(j))).isEmpty()) {
