...
size_t get_c_ab(const Set& a, const Set& b, const PWMap& map_1, const PWMap& map_2, const EdgeCost& costs, SetAF& set_fact)
{
...
unsigned get_imbalance_size(unsigned min_imbal_part, unsigned max_imbal_part, unsigned size_node_a, unsigned size_node_b,
                            unsigned current_moved_size)
{
...
CostMatrixImbalance generate_gain_matrix(const WeightedSBGraph& graph, CostMatrix& cost_matrix, unsigned partition_a_id,
                                         Partition& partition_a, unsigned partition_b_id, Partition& partition_b, unsigned LMin,
                                         unsigned LMax, SetAF& set_fact)
{
...
pair<pair<Set, Set>, pair<Set, Set>> update_sets(Partition& partition_a, Partition& partition_b, Set& current_moved_partition_a,
                                                 ...
                                                 const WeightedSBGraph& graph, SetAF& set_fact)
{
...
void update_diff(CostMatrixImbalance& cost_matrix, Partition& remaining_partition_a, Set& moved_from_partition_a,
                 ...
                 const GainObjectImbalance& gain_object, unsigned LMin, unsigned LMax, SetAF& set_fact)
{
  ...
  auto discarded_edges_a = graph.map1().preImage(affected_node_a.first).cup(graph.map2().preImage(affected_node_a.first));
  auto discarded_edges_b = graph.map1().preImage(affected_node_b.first).cup(graph.map2().preImage(affected_node_b.first));
  auto discarded_edges = discarded_edges_a.cup(discarded_edges_b);

  // if (not node_a_fully_used) {
  CostMatrixImbalance new_cost_matrix;
  for (auto g : cost_matrix) {
    if (g.a_idx == gain_object.a_idx) {
      unsigned size = min(remaining_partition_a.at(g.a_idx).cardinal(), remaining_partition_b.at(g.b_idx).cardinal());
      g.size_a = size;
      g.size_b = size;
    }

    if (g.b_idx == gain_object.b_idx) {
      unsigned size = min(remaining_partition_a.at(g.a_idx).cardinal(), remaining_partition_b.at(g.b_idx).cardinal());
      g.size_a = size;
      g.size_b = size;
    }
    ...
    g.ec_nodes_b = g.ic_nodes_b.difference(discarded_edges);

    g.ic_nodes_a = g.ic_nodes_a.difference(discarded_edges);
    g.ic_nodes_b = g.ic_nodes_b.difference(discarded_edges);

    auto ec_edges =
        g.ec_nodes_a.cup(g.ec_nodes_b).difference(g.ec_nodes_a.intersection(g.ec_nodes_b));  // disjointCup does not seem to be working
    auto ic_edges = g.ic_nodes_a.cup(g.ic_nodes_b);
...
int kl_sbg_imbalance(const WeightedSBGraph& graph, CostMatrix& cost_matrix, unsigned partition_a_id, Partition& partition_a,
                     unsigned partition_b_id, Partition& partition_b, unsigned LMin, unsigned LMax, SetAF& set_fact)
{
#if PARTITION_IMBALANCE_DEBUG
  ...
  CostMatrixImbalance gm =
      generate_gain_matrix(graph, cost_matrix, partition_a_id, partition_a, partition_b_id, partition_b, LMin, LMax, set_fact);
...
kl_sbg_partitioner_result kl_sbg_partitioner_function(const WeightedSBGraph& graph, PartitionMap& partitions, CostMatrix& cost_matrix,
                                                      unsigned LMin, unsigned LMax, vector<kl_sbg_partitioner_result>& gains,
                                                      SetAF& set_fact, MapAF& map_fact)
{
  ...
  for (size_t i = 0; i < partitions.size(); i++) {
    ...
    for (size_t j = i + 1; j < partitions.size(); j++) {
      ...
      auto gain_comp = [i, j](const kl_sbg_partitioner_result& g) { return (g.i == i and g.j == j) or (g.i == j and g.j == i); };
...
kl_sbg_partitioner_result kl_sbg_partitioner_multithreading(const WeightedSBGraph& graph, PartitionMap& partitions, unsigned LMin,
                                                            unsigned LMax, vector<kl_sbg_partitioner_result>& gains, SetAF& set_fact)
{
  // avoid repeating this
  // auto look_for_adjacents = [&set_fact](const Set& nodes, const PWMap& map1, const PWMap& map2) {
  //     auto involved_edges1 = map1.preImage(nodes);
...
void kl_sbg_imbalance_partitioner(const WeightedSBGraph& graph, PartitionMap& partitions, const float imbalance_epsilon, SetAF& set_fact,
                                  MapAF& map_fact)
{
  auto [LMin, LMax] = imbalance_epsilon > 0.0 ? compute_lmin_lmax(graph, partitions.size(), imbalance_epsilon, set_fact)
  ...
  while (change) {
    ...
    default:
      ...
      while (not gains.empty() and best_gain.gain > 0) {
        ...
        if (not gains.empty()) {
          auto max_gain_it =
              max_element(gains.begin(), gains.end(),
                          [](const kl_sbg_partitioner_result& a, const kl_sbg_partitioner_result& b) { return a.gain < b.gain; });
