...
TEST(create_sb_grpah, PartitionerTests)
{
...

    {
        ...
        auto sb_graph = sbg_partitioner::build_sb_graph("data/advection.json", set_fact, map_fact, pw_fact);

        // create nodes of the expected graph
        auto expected_nodes = set_fact.createSet();
        expected_nodes.emplaceBack(Interval(0, 1, 0));
        expected_nodes.emplaceBack(Interval(1, 1, 99));
        EXPECT_EQ(expected_nodes, sb_graph.V());
        ...
        lhs_maps.emplaceBack(map_fact.createMap(Interval(100, 1, 100), Exp(LExp(0, 0))));
        rhs_maps.emplaceBack(map_fact.createMap(Interval(100, 1, 100), Exp(LExp(1, RATIONAL(-99, 1)))));

        lhs_maps.emplaceBack(map_fact.createMap(Interval(101, 1, 198), Exp(LExp(1, RATIONAL(-100, 1)))));
        rhs_maps.emplaceBack(map_fact.createMap(Interval(101, 1, 198), Exp(LExp(1, RATIONAL(-99, 1)))));
