...
tuple<Set, Set, Set> set_piece_communication(const SetPiece& nodes, const WeightedSBGraph& graph)
{
  ...
  return {edges, non_common_edges, common_edges};
}
...
CommunicationCost::CommunicationCost(const WeightedSBGraph& graph, PartitionMap partitions)
    : ICommunicationCost(), _graph(graph), _partitions(partitions)
{
  initialize();
}
...
pair<Set, Set> CommunicationCost::get_part_communication(unsigned id)
{
  ...
  for (const auto& node : _partitions.at(id)) {
    if (_communication_by_set_piece.find(node) == _communication_by_set_piece.end()) {
      auto [edges, non_common_edges, common_edges] = internal::set_piece_communication(node, _graph);
      _communication_by_set_piece.insert({node, non_common_edges});
      _communication_by_set_piece_with_common_edges.insert({node, edges});
      _inner_edges.insert({node, common_edges});
    }
    // }
    ...
    internal_communication_partition_i = internal_communication_partition_i.cup(_inner_edges.at(node));
...
pair<Set, Set> CommunicationCost::compute_ec_ic(unsigned partition_id, const SetPiece& nodes)
{
  if (_communication_by_set_piece.find(nodes) == _communication_by_set_piece.end()) {
    ...
    _inner_edges.insert({nodes, common_edges});
  }
...
CommunicationCostSync::CommunicationCostSync(const WeightedSBGraph& graph, PartitionMap partitions)
    : ICommunicationCost(), _comm_cost(graph, partitions)
{
}
...
void set_communication_cost(CommunicationCostPtr&& cost_matrix) { internal::cost_matrix = move(cost_matrix); }
